...
import { Hand, LogOut, Plus, Trash2 } from "lucide-react";
    ...
    <div className="h-screen bg-stone-50 dark:bg-stone-950 flex flex-col">
      ...
      {isGlobalLoading && (
        <div className="fixed inset-0 bg-stone-900/60 z-50 flex items-center justify-center backdrop-blur-sm animate-in fade-in duration-300">
          <div className="flex flex-col items-center gap-4 p-8 bg-white dark:bg-stone-900 rounded-2xl shadow-2xl border border-stone-200 dark:border-stone-800 max-w-xs text-center">
      ...
      {/* Header */}
      <header className="px-4 py-3 border-b border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900 flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-1.5 text-stone-400 hover:text-stone-700 dark:hover:text-stone-300 rounded hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors"
            title="ログアウト"
          >
      ...
      {/* Main Content */}
      <div
        className={`flex-1 flex flex-col lg:flex-row gap-3 p-3 min-h-0 overflow-auto lg:pb-3 ${isGlobalLoading ? "pointer-events-none opacity-50" : ""}`}
      >
        {/* Mobile Hint (Visible only on mobile when no node selected) */}
        {!selectedNode && (
          <div className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center justify-center gap-2 py-2 px-6 bg-white/90 dark:bg-stone-900/90 backdrop-blur shadow-lg border border-blue-100 dark:border-blue-900/50 text-blue-600 dark:text-blue-400 rounded-full text-sm animate-pulse">
        ...
        <div className="flex-1 bg-white dark:bg-stone-900 rounded-lg border border-stone-200 dark:border-stone-800 overflow-hidden min-h-[300px] lg:min-h-0">
          <TreeList onNodeSelect={handleSelectNode} version={version} onRefresh={refreshTree} />
        </div>
